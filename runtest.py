"""
runtest.py — Test runner for detector.py (transcript-only build)
"""
...
def run(video_path: str, student_id: str = None, interviewee_name: str = None):
    ...
    if interviewee_name:
        print(f"  Target  : {interviewee_name}")
    ...
    if result.video_analysis:
        ...
        print(f"  Camera Score : {va.get('overall_camera_score')}/10")
        print(f"  Light Score  : {va.get('overall_lighting_score')}/10")
        print(f"  Video Summary: {va.get('overall_summary')}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python runtest.py <video_path> [student_id] [interviewee_name]")
        sys.exit(1)
